...
JOB_BASE="https://gcsweb-ci.apps.ci.l2s4.p1.openshiftapps.com/gcs/origin-ci-test/pr-logs/"
...
function upload_horreum() {
    ...
    info "Getting pass/fails for $f from Horreum"
...
for job in "baseline-pr"; do
    ###job_history="$JOB_BASE/periodic-ci-openshift-pipelines-performance-master-$job"
    job_history="$JOB_BASE/pull/redhat-performance_backstage-performance/19/pull-ci-redhat-performance-backstage-performance-main-$job/"
