...
EOF
  ...
  if [[ $policy == "$RBAC_POLICY_COMPLEX" ]]; then
    sed 's/^    //' "$WORKDIR/template/backstage/complex-rbac-config.yaml" >>"$csv_file"
  fi

  # Add orchestrator rules if needed
  if [[ "$INSTALL_METHOD" == "helm" ]] && ${ENABLE_ORCHESTRATOR:-false}; then
    sed 's/^    //' "$WORKDIR/template/backstage/helm/orchestrator-rbac-patch.yaml" >>"$csv_file"
    if [[ $policy == "$RBAC_POLICY_COMPLEX" ]]; then
      sed 's/^    //' "$WORKDIR/template/backstage/helm/complex-orchestrator-rbac-patch.yaml" >>"$csv_file"
    fi
  fi
